    ...
    [[RMStore defaultStore] requestProducts:products success:^(NSArray *products, NSArray *invalidProductIdentifiers) {
        ...
        for (SKProduct *product in products) {
            ...
            for (SKProductDiscount *discount in discounts) {
                ...
                if (discount.subscriptionPeriod != nil) {
                    subscriptionPeriod = @{
                        @"unit": [RMStore stringForPeriodUnit:discount.subscriptionPeriod.unit],
                        @"numberOfUnits": [NSString stringWithFormat:@"%lu", discount.subscriptionPeriod.numberOfUnits],
                    };
                }
                NSLocale *discountPriceLocale = discount.priceLocale;
                if (!discountPriceLocale) {
                    discountPriceLocale = product.priceLocale;
                }
                [discounts addObject:@{
                    @"identifier": NILABLE(discount.identifier),
                    @"priceLocale": NILABLE(discount.priceLocale.localeIdentifier),
                    @"numberOfPeriods": [NSString stringWithFormat:@"%lu", discount.numberOfPeriods],
                    @"type": type,
                    @"paymentMode": paymentMode,
                    @"subscriptionPeriod": NILABLE(subscriptionPeriod),
                    @"price": NILABLE([RMStore localizedPriceStringWithPrice:discount.price priceLocale:discountPriceLocale]),
                }];
            }
            NSDictionary *subscriptionPeriod = nil;
            if (product.subscriptionPeriod != nil) {
                subscriptionPeriod = @{
                    @"unit": [RMStore stringForPeriodUnit:product.subscriptionPeriod.unit],
                    @"numberOfUnits": [NSString stringWithFormat:@"%lu", product.subscriptionPeriod.numberOfUnits],
                };
            }
            [validProducts addObject:@{
                ...
                @"isDownloadable": product.isDownloadable ? @"true" : @"false",
                @"isFamilyShareable": product.isFamilyShareable ? @"true" : @"false",
                @"subscriptionGroupIdentifier": NILABLE(product.subscriptionGroupIdentifier),
                @"priceLocale": NILABLE(product.priceLocale.localeIdentifier),
                @"downloadContentVersion": NILABLE(product.downloadContentVersion),
                @"downloadContentLengths": NILABLE(product.downloadContentLengths),
                @"discounts": discounts,
                @"subscriptionPeriod": NILABLE(subscriptionPeriod)
            }];
        }
...
// Those are from old plugin:


- (void)getProducts:(CDVInvokedUrlCommand *)command {
    [self billingGetAllProductInfo:command];
}

- (void)buy:(CDVInvokedUrlCommand *)command {
    //[self billingPurchase:command];
    id productId = [command.arguments objectAtIndex:0];
    if (![productId isKindOfClass:[NSString class]]) {
        CDVPluginResult *pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsString:@"ProductId must be a string"];
        [pluginResult setKeepCallbackAsBool:YES];
...
- (void)restorePurchases:(CDVInvokedUrlCommand *)command {
    //[self billingRestorePurchases:command];
    [[RMStore defaultStore] restoreTransactionsOnSuccess:^(NSArray *transactions){
        NSMutableArray *validTransactions = [NSMutableArray array];
        NSMutableDictionary *result = [NSMutableDictionary dictionary];
        NSDateFormatter *formatter = [[NSDateFormatter alloc] init];
...
- (void)getReceipt:(CDVInvokedUrlCommand *)command {
    //[self billingGetReceipt:command];
    [[RMStore defaultStore] refreshReceiptOnSuccess:^{
        NSURL *receiptURL = [[NSBundle mainBundle] appStoreReceiptURL];
        NSData *receiptData = [NSData dataWithContentsOfURL:receiptURL];
        NSString *encReceipt = [receiptData base64EncodedStringWithOptions:0];
